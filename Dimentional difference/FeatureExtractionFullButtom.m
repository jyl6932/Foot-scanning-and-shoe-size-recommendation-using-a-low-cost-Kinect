function [features] = FeatureExtractionFullButtom(ptCloud,FileName)
...
%% 40mm 1st & 2nd
%Kinect dont have enough points here
groupUpper = upper_boundary(find((upper_boundary(:,1) >= 39) & (upper_boundary(:,1) <= 41) ),:);
groupLower = lower_boundary(find((lower_boundary(:,1) >= 39) & (lower_boundary(:,1) <= 41) ),:);
first_40mm = median(maxk(groupUpper(:,2),5)); 
second_40mm = median(mink(groupLower(:,2),5));
...
%% P4 
[upper_boundary, lower_boundary] = DrawBoundary_feedback_boundary(ptCloud.Location,3);
...
FootLength = median(maxk(ptCloud20.Location(:,1),10)) 
